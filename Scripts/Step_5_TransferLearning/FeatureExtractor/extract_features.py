...
class CustomDatasetLoader:
    ...
    def load_data(self):
        ...
        for class_name, data_dir in [('benign', self.benign_dir), ('malware', self.malware_dir)]:
            ...
            for filename in files:
                ...
                try:
                    img_array = self.converter.convert_to_image(file_path) #Conversão do arquivo para imagem. Aqui o arquivo .exe ou .json é transformado em uma imagem 32 × 32 × 3. tem que utilizar classifier.extract_features(img_array)
    ...
    def save_as_libsvm_bruto(self, output_file):
            ...
            for class_name, data_dir in [('benign', self.benign_dir), ('malware', self.malware_dir)]:
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        features = img_array.reshape(-1)  # <--- AQUI ESTÁ O ERRO: Ainda são os 3072 pixels brutos!
    ...
    def save_as_csv_bruto(self, output_file, delimiter=';'):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            ...
            for class_name, data_dir in [('benign', self.benign_dir), ('malware', self.malware_dir)]:
                ...
                for filename in files:
                    file_path = os.path.join(data_dir, filename)
                    try:
                        img_array = self.converter.convert_to_image(file_path) # CSV está recebendo: os 3072 valores dos pixels da imagem 32×32×3 e não as características extraídas pelo ResNet.
                        features = img_array.reshape(-1)  # <--- AQUI ESTÁ O ERRO: Ainda são os 3072 pixels brutos!
...
#Agora estou utilizando os metodos que vai utilizar as redes pretreinada
    def save_as_libsvm_streaming(self, output_file, classifier):
        """Salva dataset em formato LIBSVM usando features da Rede Neural, sem estourar a memória."""
        os.makedirs(os.path.dirname(output_file) or '.', exist_ok=True)

        with open(output_file, 'w', encoding='utf-8') as f:
            for class_name, data_dir in [('benign', self.benign_dir), ('malware', self.malware_dir)]:
                label = 0 if class_name == 'benign' else 1  # LIBSVM prefere 0 e 1
                if not os.path.exists(data_dir):
                    continue
                files = [fn for fn in os.listdir(data_dir) if os.path.isfile(os.path.join(data_dir, fn))]
                ...
                for filename in files:
                    file_path = os.path.join(data_dir, filename)
                    try:
                        # 1. Converte o arquivo em imagem 32x32x3
                        img_array = self.converter.convert_to_image(file_path)
                        
                        # 2. A MÁGICA: Extrai as características usando a Rede Neural
                        features = classifier.extract_features(img_array)[0] 
                        
                        # 3. Monta a linha LIBSVM com features float
                        line = str(label)
                        for j, value in enumerate(features):
                            if value != 0:
                                line += f" {j+1}:{value:.6f}"  # 6 casas decimais para precisão
                        f.write(line + '\n')
                    except Exception as e:
                        print(f"Erro ao processar {file_path}: {e}")

        print(f"✅ LIBSVM salvo em: {output_file} usando features da rede neural (Streaming).")

    def save_as_csv_streaming(self, output_file, classifier, delimiter=';'):
        """Salva dataset em CSV usando features da Rede Neural, sem estourar a memória."""
        out_dir = os.path.dirname(output_file)
        if out_dir:
            os.makedirs(out_dir, exist_ok=True)

        num_features = None  # Será descoberto dinamicamente na primeira extração
            ...
            for class_name, data_dir in [('benign', self.benign_dir), ('malware', self.malware_dir)]:
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        # 3. Define o cabeçalho dinamicamente na primeira iteração
                        if num_features is None:
                            num_features = len(features)
                            header = ['app', 'output'] + [f'feature_{i+1}' for i in range(num_features)]
                            writer.writerow(header)
                        
                        # 4. Salva a linha com features float arredondadas
                        row = [filename, label] + [round(float(v), 6) for v in features]
                        writer.writerow(row)
                    except Exception as e:
                        print(f"Erro ao processar {file_path}: {e}")

        print(f"✅ CSV salvo em: {output_file} com {num_features} features extraídas pela rede neural (Streaming).")
...
class Classifier:
    def __init__(self, model_name='lenet'):
        ...
        self.model = self.model_defs[model_name](load_weights=True) #Isso carrega a rede pré-treinada, por exemplo: -model resnet
        self.class_names = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']

        # Feature extractor: penultimate layer output
        # definição do extrator de características, usando a penúltima camada da CNN.
        self.feature_model = tf.keras.Model(
            inputs=self.model._model.input,
            outputs=self.model._model.layers[-2].output
        )

    def extract_features(self, image): #extrai as características da imagem usando a rede pré-treinada.
...
if __name__ == '__main__':
    ...
    # Saídas
    #Adicionar libsvm#parser.add_argument('-libsvm_file', default='FeatureExtractor/TransferLearningAntivirus.libsvm',
                        #Adicionar libsvm#help='Path to save dataset in LIBSVM format.')
    ...
    # Load dataset
    # Inicializa o loader (mas NÃO carrega tudo na memória)
    loader = CustomDatasetLoader(args.data_benign, args.data_malware)
    #comentado#x_data, y_data, file_names = loader.load_data()
    ...
    # Inicializa o classificador (carrega a ResNet ou outro modelo na memória)
    print(f"\nInicializando modelo {args.model.upper()} para extração de características...")
    classifier = Classifier(args.model)
    print("\nIniciando processamento em streaming (arquivo por arquivo)...")
    
     # Salva em LIBSVM
    #print(f"\nSalvando dataset em formato LIBSVM: {args.libsvm_file}")
    #loader.save_as_libsvm_streaming(args.libsvm_file, classifier)
    
    # Salva em CSV
    print(f"\nSalvando dataset em formato CSV: {args.csv_file}")
    loader.save_as_csv_streaming(args.csv_file, classifier, delimiter=';')
    
    print("\n🎉 Processo concluído com sucesso! Memória otimizada e features reais da CNN extraídas.")

"""
    classifier = Classifier(args.model) #cria classificador
    ...
    # comentado#indices = [args.image_idx] if args.image_idx is not None else range(len(x_data))
...
"""  
    ...
    # Salva direto em streaming
    #Adicionar libsvm#print(f"\nSaving dataset in LIBSVM format to: {args.libsvm_file}")
    #Adicionar libsvm#loader.save_as_libsvm_streaming(args.libsvm_file)
"""    
